...
Write-Host Creating a Restore Point
	...
	ShowBalloonTipInfo ("Completed Building Restore Point : ","Restore Point AlanisAWESOMERestorePoint has been completed.")
...
Write-Host "Backing Up Registry Data to C:\Registry Data"
	...
	ShowBalloonTipInfo ("Completed Backing Up Registry : ","Entire registry has been backed up to C:\Registry Backup\.")
...
do
 {
     ...
     switch ($selection)
     {
		 ...
		 ShowBalloonTipInfo ("Did you know that yawning cools your brain off? : ","It's true, but only if the air ouside is cooler than your body temperature.")
		...
		New-ItemProperty -Path HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\TaskManager -Name Preferences -PropertyType Binary -Value $Preferences -Force
		 ...
		 ShowBalloonTipInfo ("Completed with the removal of bloatware!","You know what to do!")
	    ...
	    ShowBalloonTipInfo ("Finished Installing!","Chrome and MSEdgeRedirect have been installed.")
         } '3' {
            ...
            $LocalTempDir = $env:TEMP; $MSEdgeRedirect = "MSEdgeRedirect.exe"; (new-object    System.Net.WebClient).DownloadFile('https://github.com/rcmaehl/MSEdgeRedirect/releases/latest/download/MSEdgeRedirect.exe', "$LocalTempDir\$MSEdgeRedirect"); & "$LocalTempDir\$MSEdgeRedirect"; $Process2Monitor =  "MSEdgeRedirect"; Do { $ProcessesFound = Get-Process | ?{$Process2Monitor -contains $_.Name} | Select-Object -ExpandProperty Name; If ($ProcessesFound) { "Still running: $($ProcessesFound -join ', ')" | Write-Host; Start-Sleep -Seconds 2 } else { rm "$LocalTempDir\$MSEdgeRedirect" -ErrorAction SilentlyContinue -Verbose } } Until (!$ProcessesFound)
	    ShowBalloonTipInfo ("Finished Installing!","Firefox and MSEdgeRedirect have been installed.")
         } '4' {
            Write-Host Setting Windows Dark Mode 
            #Dark Mode
            Set-ItemProperty -Path HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize -Name SystemUsesLightTheme -Value 0 -Type Dword -Force
	    ShowBalloonTipInfo ("Dark Mode Initialized","Autobots Roll Out!")
        } '5' {
	    ...
	    ShowBalloonTipInfo ("Light Mode Initialized","Decepticons Transform & Rise Up")
        } '6' {
	    ...
	    ShowBalloonTipInfo ("Finished Installing","Live Tiles Anywhere, Standing By...")
        } '7' {
	    ...
	    ShowBalloonTipInfo ("Finished Installing","Quick Assist Standing By")
        } '8' {
